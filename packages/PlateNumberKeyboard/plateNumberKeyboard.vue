    ...
    <div class="keyboard-content" v-show="innerKeyboardVisible">
      ...
      <div class="keyboard-prefix" v-if="curKeyIdx === 1">
        ...
        <div class="keyboard-row">
          ...
          <div class="keyboard-key keyboard-key__del" @touchstart="handleDelTouch">
            <span class="keyboard-key__txt"></span>
          </div>
          <div class="keyboard-key keyboard-key__empty">
            <span class="keyboard-key__txt"></span>
      ...
      <div class="keyboard-suffix" v-else>
        ...
        <div class="keyboard-row">
          <div
            class="keyboard-key"
            :class="[isLiterCom(key) && 'keyboard-key__disabled', isToO(key) && 'keyboard-key__disabled']"
            @touchstart="handleKeyTouchStart(key, $event)"
            @touchend="handleKeyTouchEnd"
      ...
      <div class="keyboard-key__hover" v-show="keyHover" :style="{ top: keyHoverY + 'px', left: keyHoverX + 'px' }">
        {{ keyVal }}
      </div>
    </div>
  </div>
...
<script>
import { isLiter, isPlateNum, getVertexPosition, isMobile } from '../utils';
...
export default {
  ...
  methods: {
    ...
    handleKeyTouchStart(key, e) {
      ...
      this.keyHover = true;
      const { top, left } = getVertexPosition(e.target);
...
.keyboard {
  &-input {
    ...
    &__energy {
      ...
      &--icon {
        ...
        width: 64px;
        height: 28px;
        background: url('https://cabin-cdn.oss-cn-hangzhou.aliyuncs.com/img/energy-icon.png') no-repeat center/64px 28px;
        z-index: 10;
      }
    }
    &__active {
  ...
  &-key {
    ...
    &__del {
      .keyboard-key__txt {
        height: 90px;
        background: #fff url('https://cabin-cdn.oss-cn-hangzhou.aliyuncs.com/img/del-icon.png') no-repeat 50%;
    ...
    &__hover {
      ...
      z-index: 50;
      background: transparent url('https://cabin-cdn.oss-cn-hangzhou.aliyuncs.com/img/key-tip.png') no-repeat 50% / auto
        100%;
    }
